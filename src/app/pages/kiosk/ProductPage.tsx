...
export function ProductPage({ products, cart, onUpdateCart, onCheckout, onExit }: ProductPageProps) {
  ...
  return (
    <div className="flex flex-col h-full bg-background text-primary overflow-hidden font-sans">

      {/* ── Compact Header ── */}
      <header className="bg-primary text-white flex items-center justify-between px-4 py-2.5 z-20 shadow-md flex-shrink-0">
        <div className="flex items-center gap-2.5 min-w-0">
          ...
          <div className="min-w-0 leading-tight">
            ...
            <p className="text-[9px] text-accent font-bold uppercase tracking-widest">Premium Selection</p>
          </div>
        </div>
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          >
            ...
            <span>Cart</span>
            {cart.length > 0 && (
              <span className="absolute -top-1 -right-1 h-4 w-4 flex items-center justify-center rounded-full bg-accent text-primary text-[9px] font-black border border-primary">
                {cart.length}
              </span>
            )}
          </button>
                ...
                {product ? (
                  <>
                    ...
                    <div className="relative bg-slate-50 overflow-hidden" style={{ height: '70%' }}>
                      ...
                      {/* Price badge */}
                      <span className="absolute top-2 right-2 bg-primary text-white text-xs font-black px-2.5 py-1 rounded-full shadow-md z-10">
                        ₱{product.price}
                      </span>
                      {/* In-cart indicator */}
                      {cartItem && (
                        <span className="absolute top-2 left-2 bg-primary text-white text-[10px] font-black px-2 py-1 rounded-full flex items-center gap-1 shadow-md z-10">
      ...
      <motion.div
        ...
        className="bg-primary text-white flex items-center justify-between px-4 py-2.5 flex-shrink-0 shadow-[0_-4px_20px_rgba(0,0,0,0.2)]"
      >
        {/* Summary pills */}
        <div className="flex items-center gap-3">
          ...
          <div className="text-center">
            ...
            <p className="text-base font-black tabular-nums text-accent leading-tight">₱{totalAmount.toLocaleString()}</p>
          </div>
        </div>
        {/* Action buttons */}
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            onClick={onCheckout}
            className="h-9 px-4 rounded-xl bg-accent text-primary text-xs font-black flex items-center gap-1 disabled:opacity-40 transition-all active:scale-95 shadow-md"
          >
          ...
          <>
            ...
            >
              ...
              <ScrollArea className="flex-1 px-4 py-3">
                {cart.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-16 text-slate-300 gap-4">
                    ...
                    <button className="text-xs font-bold text-primary underline underline-offset-2" onClick={() => setIsCartOpen(false)}>
                      Start Selection
                    </button>
                  </div>
                ) : (
                    ...
                    {cart.map((item, index) => (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0 flex flex-col justify-center">
                          ...
                          <p className="font-black text-primary text-base leading-tight mt-0.5">₱{(item.product.price * item.quantity).toLocaleString()}</p>
                        </div>
                        <button
              ...
              <div className="px-4 py-4 border-t border-slate-100 bg-white flex-shrink-0">
                ...
                <Button
                  className="w-full h-12 text-base font-black bg-primary hover:bg-secondary rounded-2xl shadow-lg transition-all active:scale-[0.98]"
                  disabled={cart.length === 0}
      ...
      <AnimatePresence>
        {selectedProduct && (
          ...
          >
            ...
            >
              {/* Image strip */}
              <div className="relative flex-shrink-0 bg-gradient-to-br from-primary to-primary" style={{ height: '240px' }}>
                ...
                <div className="absolute bottom-0 left-0 right-0 px-5 pb-4 pt-8 bg-gradient-to-t from-primary to-transparent">
                  <h2 className="text-2xl font-black text-white tracking-tighter leading-none">{selectedProduct.name}</h2>
                  <div className="flex items-center gap-2 mt-1.5">
                    <Badge className="bg-accent text-primary font-black px-3 py-0.5 rounded-full text-sm">₱{selectedProduct.price}</Badge>
              ...
              <div className="p-5 flex flex-col gap-4 flex-1">
                ...
                <div className="flex items-center justify-between bg-slate-50 rounded-2xl px-4 py-3 border border-slate-100">
                  ...
                  <span className="text-xl font-black text-primary tracking-tighter">₱{(selectedProduct.price * quantity).toLocaleString()}</span>
                </div>

                {/* Add button */}
                <Button
                  className="w-full h-13 text-base font-black bg-accent text-primary hover:bg-accent/80 rounded-2xl shadow-lg transition-all active:scale-[0.98]"
