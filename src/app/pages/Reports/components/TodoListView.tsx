    ...
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        {sorted.map(todo => {
          ...
          return (
            <div key={todo.id} className={`bg-card border rounded-xl p-4 transition-all ${todo.completed ? 'border-border opacity-60' : 'border-indigo-200/70'}`}>
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-2 mb-1">
                    ...
                    <button onClick={() => onDelete(todo.id)} className="p-1 rounded-md hover:bg-red-50 text-muted-foreground hover:text-red-500 transition-colors cursor-pointer shrink-0" title="Delete to-do">
                      <Trash2 size={14} />
                    </button>
                  </div>
