          ...
          {lowStockMachines.length > 0 ? (
            <div className="space-y-3 py-6">
              {lowStockMachines.map(machine => {
                ...
                return (
                  <div key={machine.id} className="group p-4 border border-slate-100 rounded-2xl bg-slate-50/50 hover:bg-white hover:border-primary/20 hover:shadow-sm transition-all duration-300">
                    <div className="flex items-center justify-between mb-3">
                      <div>
                        <p className="font-bold text-sm text-slate-900 tracking-tight">{machine.name}</p>
                        <p className="text-xs font-medium text-slate-500">{machine.location}</p>
                      </div>
            ...
            <div className="flex flex-col items-center justify-center h-full text-slate-400 py-12">
              ...
              <p className="font-bold text-slate-900">Inventory Optimal</p>
              <p className="text-sm font-medium text-slate-500 mt-1">All machines are well supplied</p>
            </div>
          )}
          </ScrollArea>
        </CardContent>
